...
describe('MyEnrollmentsComponent - Tests Simples', () => {
  ...
  beforeEach(async () => {
    ...
    apiServiceMock = {
      getStudentEnrollmentsWithDetails: vitest.fn().mockReturnValue(of([])),
      deleteEnrollment: vitest.fn().mockReturnValue(of({}))
    };
  ...
  describe('Funcionalidad básica', () => {
    it('debería poder llamar cancelEnrollment con un objeto EnrollmentWithDetails', () => {
      // Test: Verificamos que podemos llamar el método con parámetros correctos
      const mockEnrollment = { id: '1' } as any;
